...
def connect_to_obs(requestData):
    fields = ["ip", "password", "port"]
    for field in fields:
        if field not in requestData:
            return fail_response(f"Your request is missing the {field}.")
    return simpleobsws.WebSocketClient(
        url=f"ws://{requestData['ip']}:{requestData['port']}",
...
async def request_callback(request, emit):
    ...
    logging.info(
        "Performing request for request type `{}` | Emit: {} | Client IP: {}".format(
            requestType, emit, request.remote
        )
    )
    logging.debug("Request data:\n{}".format(requestData))

    ws = connect_to_obs(requestData)
...
async def init():
    ...
    try:
        ...
        logging.error(
            "Failed to connect to the obs-websocket server. Got connection refused."
        )
        return False
    if not await ws.wait_until_identified():
        logging.error(
            "Identification with obs-websocket timed out. Could it be using 4.x?"
        )
        return False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="A Python-based program that provides HTTP endpoints for obs-websocket"
    )
    parser.add_argument(
        "--http_bind_addres",
        dest="http_bind_addres",
        default=config.get("http", "bind_to_address", fallback="0.0.0.0"),
    )
    parser.add_argument(
        "--http_bind_port",
        dest="http_bind_port",
        type=int,
        default=config.getint("http", "bind_to_port", fallback=4445),
    )
    parser.add_argument(
        "--http_auth_key",
        dest="http_auth_key",
        default=config.get("http", "authentication_key", fallback=""),
    )
    # parser.add_argument(
    #     "--ws_url",
    #     dest="ws_url",
    #     default=config.get("obsws", "ws_url", fallback="ws://127.0.0.1:4444"),
    # )
    # parser.add_argument(
    #     "--ws_password",
    #     dest="ws_password",
    #     default=config.get("obsws", "ws_password", fallback=""),
    # )
    ...
    if httpAuthKey:
        logging.info(
            "HTTP server will start with AuthKey set to `{}`".format(httpAuthKey)
        )
    else:
